<script lang="ts">
  ...
  let { spacing, rangeValue, data, colorSelection, media, mediaFilter, avgEnabled, target, barFilterSelection, displayBarFilter = $bindable(), barValue = $bindable() } = $props();
</script>
  ...
  <!-- Componente grafico -->
  <Chart {data} {rangeValue} {spacing} {colorSelection} {media} {mediaFilter} {barFilterSelection} bind:displayBarFilter = {displayBarFilter} bind:barValue = {barValue} {controlTarget} {avgEnabled}/>
